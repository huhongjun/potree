...
Potree.Classification = {
	'DEFAULT': {
		...
		13:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		14:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		15:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		16:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		17:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		18:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		19:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		20:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		21:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		22:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		23:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		24:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		25:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		26:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		27:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		28:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
		29:	new THREE.Vector4(0.3, 0.6, 0.6, 0.5),
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher{
	...
	constructor(domElement, args){
		...
		this.classifications = {
			0: { visible: true, name: 'conductor' },
			1: { visible: true, name: 'underbuild' },
			2: { visible: true, name: 'other_line' },
			3: { visible: true, name: 'shield_wire' },
			4: { visible: true, name: 'high_vegetation' },
			5: { visible: true, name: 'mid_vegetation' },
			6: { visible: true, name: 'low_vegetation' },
			7: { visible: true, name: 'ground' },
			8: { visible: true, name: 'mkp_ground' },
			9: { visible: true, name: 'building' },
			10: { visible: true, name: 'temporary_building' },
			11: { visible: true, name: 'road' },
			12: { visible: true, name: 'bridges' },
			13: { visible: true, name: 'railway' },
			14: { visible: true, name: 'scissors_crossing(up)' },
			15: { visible: true, name: 'scissors_crossing(down)' },
			16: { visible: true, name: 'structures' },
			17: { visible: true, name: 'carrier' },
			18: { visible: true, name: 'river' },
			19: { visible: true, name: 'navigable' },
			20: { visible: true, name: 'unnavigable' },
			21: { visible: true, name: '21' },
			22: { visible: true, name: 'insulator' },
			23: { visible: true, name: 'other' },
			24: { visible: true, name: 'noise' },
			25: { visible: true, name: 'substations' },
			26: { visible: true, name: 'Ship& Car' },
			27: { visible: true, name: 'lake' },
			28: { visible: true, name: 'Drainage_thread' },
			29: { visible: true, name: 'notype' }
		};
	...
	update (delta, timestamp) {
		...
		for (let pointcloud of this.scene.pointclouds) {	// 更新每个点云的独立配置
	...
	loop (timestamp) {
		...
		if (this.useEDL && Potree.Features.SHADER_EDL.isSupported()) {	//最终渲染
			if (!this.edlRenderer) {
...
initSidebar = (viewer) => {
	...
	function initClassificationList () {
		...
		addClassificationItem(0, '电力线缆'); 		//conductor
		addClassificationItem(1, '未知'); 		//underbuild
		addClassificationItem(2, '其他线缆'); 		//other_line
		addClassificationItem(3, '电力导线'); 		//shield_wire
		addClassificationItem(4, '高植被'); 		//high_vegetation
		addClassificationItem(5, '中植被'); 		//mid_vegetation
		addClassificationItem(6, '低植被'); 		//low_vegetation
		addClassificationItem(7, '地面'); 		//ground
		addClassificationItem(8, 'mkp地面'); 		//mkp_ground
		addClassificationItem(9, '建筑'); 		//building
		addClassificationItem(10, '临时建筑'); 		//temporary_building
		addClassificationItem(11, '道路'); 		//road
		addClassificationItem(12, '桥梁'); 		//bridges
		addClassificationItem(13, '铁路'); 		//railway
		addClassificationItem(14, '交跨(上)'); 		//scissors_crossing(up)
		addClassificationItem(15, '交跨(下)'); 		//scissors_crossing(down)
		addClassificationItem(16, '电力杆塔'); 		//structures
		addClassificationItem(17, 'carrier'); //conductor
		addClassificationItem(18, '江河'); //conductor
		addClassificationItem(19, 'navigable'); //conductor
		addClassificationItem(20, 'unnavigable'); //conductor
		addClassificationItem(21, '21'); //conductor
		addClassificationItem(22, 'insulator'); //conductor
		addClassificationItem(23, 'other'); //conductor
		addClassificationItem(24, '噪点'); //conductor
		addClassificationItem(25, '变电站'); //conductor
		addClassificationItem(26, 'Ship& Car'); //conductor
		addClassificationItem(27, '湖'); //conductor
		addClassificationItem(28, 'Drainage_thread'); //conductor
		addClassificationItem(29, '无类型'); //conductor
	...
	function initMeasurementDetails () {
		...
		let createAttributesTable = (measurement) => {
			...
			if (point.color) {
				...
				text = 'RGB：' + text +',分类：'+ point.classification +',强度：'+point.intensity+'，回波：'+point.numberOfReturns;
				elTable.append($(`
					<tr>
						<td>${text}</td>
					</tr>
	...
	function initSceneList () {
		...
		let initUIElements = function (i) {
			...
			let updateMaterialPanel = function (event, ui) {
				...
				console.log(pcMaterial.pointColorType);
